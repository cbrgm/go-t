...
func retweetsStatusAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t status retweets`")
	}
	...
	opts := &twitter.RetweetsStatusOptions{
		ID:       tweetID,
		AsList:   c.Bool("list"),
