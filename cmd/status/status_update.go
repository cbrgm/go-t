...
func updateStatusAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t status update`")
	}

	status, err := in.ParseTweetStatusFromArgs(c.Args()[0], c.Bool("file"))
	if err != nil {
		out.Fatal(err)
	}

	opts := &twitter.UpdateStatusOptions{
		Status:  status,
		AsList:  c.Bool("list"),
	...
	if !in.AskForConfirmation("@"+cfg.Context+": update status, continue?", opts.SkipYes) {
		out.Fatal("user aborted action")
	}

	client := twitter.NewFromConfig(accCfg, g.Debug)
	client.UpdateTweet(opts)
}
