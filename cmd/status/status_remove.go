...
func removeStatusAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t status remove`")
	}
	...
	opts := &twitter.RemoveStatusOptions{
		ID:       tweetID,
		AsList:   c.Bool("list"),
	...
	if !in.AskForConfirmation("@"+cfg.Context+": delete tweet, continue?", opts.SkipYes) {
		out.Fatal("user aborted action")
	}

	client := twitter.NewFromConfig(accCfg, g.Debug)
	client.RemoveTweet(opts)
}
