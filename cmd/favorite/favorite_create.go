...
func createFavoriteAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t favorites like`")
	}
	...
	opts := &twitter.CreateFavoriteOptions{
		ID:      tweetID,
		AsList:  c.Bool("list"),
	...
	if !in.AskForConfirmation("@"+cfg.Context+": like tweet, continue?", opts.SkipYes) {
		out.Fatal("user aborted action")
	}

	client := twitter.NewFromConfig(accCfg, g.Debug)
	client.CreateFavorite(opts)
}
