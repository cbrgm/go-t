...
func removeFavoriteAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t favorites dislike`")
	}
	...
	opts := &twitter.RemoveFavoriteOptions{
		ID:      tweetID,
		AsList:  c.Bool("list"),
	...
	if !in.AskForConfirmation("@"+cfg.Context+": dislike tweet, continue?", opts.SkipYes) {
		out.Fatal("user aborted action")
	}

	client := twitter.NewFromConfig(accCfg, g.Debug)
	client.RemoveFavorite(opts)
}
