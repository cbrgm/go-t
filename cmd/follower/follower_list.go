...
func listFollowersAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t followers`")
	}
	...
	opts := &twitter.ListFollowersOptions{
		ScreenName: name,
		UserID:     userID,
		Head:       c.Int("head"),
		Tail:       c.Int("tail"),
