...
func createFriendshipAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t friendship follow`")
	}
	...
	opts := &twitter.CreateFriendshipOptions{
		ScreenName: name,
		UserID:     userID,
		Follow:     true,
	...
	if !in.AskForConfirmation("@"+cfg.Context+": follow user "+opts.ScreenName+" , continue?", opts.SkipYes) {
		out.Fatal("user aborted action")
	}

	client := twitter.NewFromConfig(accCfg, g.Debug)
	client.CreateFriendship(opts)
}
