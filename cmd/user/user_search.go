...
func searchUserAction(c *cli.Context) {
	if !util.IsValidArgsLength(c.Args(), 1) {
		out.Fatalln("invalid number of arguments passed for `t users search`")
	}

	opts := &twitter.UserSearchOptions{
		Query:  c.Args()[0],
		AsList: c.Bool("list"),
